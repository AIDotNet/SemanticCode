...
public class ClaudeCodeSettingsViewModel : ViewModelBase
{
    ...
    public ClaudeCodeProfileInfo? SelectedProfile
    {
        ...
        set
        {
            ...
            if (value != null)
            {
                _ = Task.Run(() => SwitchToProfileAsync(value.Name));
            }
        }
    }
    ...
    public ClaudeCodeSettingsViewModel()
    {
        ...
        CreateProfileCommand = ReactiveCommand.CreateFromTask(CreateProfileAsync, this.WhenAnyValue(x => x.NewProfileName).Select(name => !string.IsNullOrWhiteSpace(name)));
        DeleteProfileCommand = ReactiveCommand.CreateFromTask(DeleteProfileAsync, this.WhenAnyValue(x => x.SelectedProfile).Select(profile => profile != null && profile.Name != "default"));
        SetDefaultProfileCommand = ReactiveCommand.CreateFromTask(SetDefaultProfileAsync, this.WhenAnyValue(x => x.SelectedProfile).Select(profile => profile != null && !profile.IsDefault));
        DuplicateProfileCommand = ReactiveCommand.CreateFromTask(DuplicateProfileAsync, this.WhenAnyValue(x => x.SelectedProfile).Select(profile => profile != null));
